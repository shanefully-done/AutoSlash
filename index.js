...
if (!GEMINI_API_KEY || !SLASHPAGE_API_KEY || !TARGET_URL) {
  console.error('Missing one or more required environment variables: GEMINI_API_KEY, SLASHPAGE_API_KEY, TARGET_URL');
  process.exit(1);
}
...
async function generateContent(data) {
  ...
  const prompt = `# 🧠 SEO Blog Writing Guideline: Daily SaaS Product Reviews

Use this guide to craft engaging, SEO-optimized blog posts reviewing new SaaS products discovered daily. Your writing must follow the structure, tone, and format below. At the end of the guideline, there will be a HTML content that you will use to generate the blog post.
...
\`\`\`markdown
## Today’s Standout SaaS Products

### 🚀 [Product Name](https://link-to-product.com)
Briefly explain what the product does, who it’s for, and what makes it stand out. Mention any unique features or use cases.

### 🔍 [Product Name](https://link-to-product.com)
Explain the product in simple terms. Highlight any interesting value proposition or differentiator.
...
- Use **headings** and **subheadings** to make scanning easier.
...
async function publish(title, body) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Failed to publish: ${res.status} ${res.statusText} - ${errorText}`);
  }
